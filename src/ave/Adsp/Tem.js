...
export const xf = (value) => {
	...
	if(imgsize === '4'){
		return `<div class="avedsp-situ avedsp-common"><div class="situ-cnt">
			...
			}).join('')}<a class="avedsp-icon">广告<i>：${value.source.slice(0, 4)}</i></a><span class="avedsp-hideicon"></span></div>`.trim();
	}else{
		return `<div class="avedsp-datu avedsp-common"><a class="datu-cnt" href="${value.url}" target="_blank" title="${value.topic.split("&&")[0]}"><img src="${value.miniimg[0].src}"  width="300" height="250" border="0" data-baiduimageplus-ignore /></a><a class="avedsp-icon">广告<i>：${value.source.slice(0, 4)}</i></a><span class="avedsp-hideicon"></span></div>`.trim();
	}
}
...
export const th = (value) => {
	...
	if(imgsize === '4'){
		return `<div class="avedsp-situ avedsp-common"><div class="situ-cnt">
			${[0,1,2,3].map((item,index)=>{
				return `<div class="situ-item ${index==1||index==3 ? 'nomar':''}"><div class="situ-img"><a href="${value.url}" target="_blank"><img src="${value.miniimg[index] ? value.miniimg[index].src : value.miniimg[0].src}" alt=""></a></div><a class="situ-text" href="${value.url}" target="_blank">${topicArr[index] ? topicArr[index]:topicArr[0]}</a></div>`.trim();
			}).join('')}<a class="avedsp-icon">广告<i>：${value.source.slice(0, 4)}</i></a></div>`.trim();
	}else{
...
export const sw = (value) => {
	...
	if(Number(value.adtype) === 101){
		return `<div class="avedsp-fwdatu avedsp-common"><a class="fwdatu-cnt" href="${value.url}" target="_blank">
				<h3>${value.topic.split('&&')[0]}</h3><p class="fwdatu-img"><img src="${value.miniimg[0].src}" /></p><p class="fwdatu-from">${value.source}<span>广告</span></p></a></div>`.trim();
	}else if(imgsize >= 4){
